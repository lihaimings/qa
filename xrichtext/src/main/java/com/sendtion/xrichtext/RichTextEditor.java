...
public class RichTextEditor extends ScrollView {
    ...
    private ArrayList<String> videoPaths; //视频地址集合
...
//                    }
    ...
    public EditText createEditText(String hint, int paddingTop) {
        ...
        editText.setHintTextColor(getColorStateList(getContext(), R.color.color_e8e8e8));
        ...
        editText.setOnFocusChangeListener(onFocusAutoClearHintListener());
        return editText;
    }
    ...
    public void insertVideo(String imagePath) {
        ...
        try {
            ...
            if (lastEditStr.length() == 0) {
                //如果当前获取焦点的EditText为空，直接在EditText下方插入图片，并且插入空的EditText
                addEditTextAtIndex(lastEditIndex + 1, "");
                addVideoViewAtIndex(lastEditIndex + 1, imagePath);
            } else if (editStr1.length() == 0) {
                //如果光标已经顶在了editText的最前面，则直接插入图片，并且EditText下移即可
                addVideoViewAtIndex(lastEditIndex, imagePath);
                //同时插入一个空的EditText，防止插入多张图片无法写文字
                addEditTextAtIndex(lastEditIndex + 1, "");
            } else if (editStr2.length() == 0) {
                // 如果光标已经顶在了editText的最末端，则需要添加新的imageView和EditText
...
//            imageView.getLayoutParams().height = getScreenWidth(getContext()) *(bitmap.getHeight()/bitmap.getWidth());
    ...
    public byte[] string2byte(String strByte) {
        if (strByte.isEmpty()) {
            return null;
        }
        ...
        try {
            ...
            for (int index = 0; index < num; index++) {
                ...
                if (itemView instanceof EditText) {
                    ...
                    itemData.inputStr = item.getText().toString(); //内容写到数据中
                } else if (itemView instanceof RelativeLayout) { //如果数据是图片
                    ...
                    NiceVideoPlayer item = itemView.findViewById(R.id.nice_video);
                    itemData.videoPath = item.getUrl();

                    itemData.videoPath = item.imagePath;
                    Log.d("数据","视频里的搜图"+item.imagePath);
                }
    ...
    public ColorStateList getColorStateList(Context context, @ColorRes int resId) {
